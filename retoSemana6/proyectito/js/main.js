...
/**REQUERIDA
 * 1. mostrar estos platillos de forma identica a como lo hace preview  DONE
 *
...
//capturamos el main por su ID 
let contenido = document.getElementById('contenido');

//hago el recorrido de todo el arreglo de objetos
listaPlatillos.forEach(function(platillos){

    
//creamos los elementos que usaremos y sus clases
...
div_tarjeta.appendChild(div_imagen);
...
div_tarjeta.appendChild(div_texto);

let div_precio = document.createElement("div");
div_precio.classList.add("precio");
div_tarjeta.appendChild(div_precio);

//llamo los datos del arreglo de objetos
imagen.setAttribute("src", `${platillos.imagen}`);

div_texto.innerHTML = `<h4>${platillos.nombre}</h4>
                    <p>${platillos.descripcion}</p>`;

div_precio.innerHTML = `<span>Precio: S/ ${platillos.precio}</  span>
                        <button class="btn-agregar">Agregar</button>`
contenido.appendChild(div_tarjeta);
});
